...
| `claims` | string[]                                          | N        | an array of claims that fulfill the requirements declared in an [Offering](#offering). If the creator of the RFQ wants to include no claims, then `claims` SHOULD be omitted rather than being an empty list. `claims` must not be an empty list. |
