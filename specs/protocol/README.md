...
| `claimsHash` | string                                           | N        | Salted hash of the list the claims appearing in `privateData.claims`. Omit `claimsHash` to denote that the RFQ contains no claims, i.e. `privateData.claims` is omitted. If `privateData.claims` is an empty list, `claimsHash` is the salted hash of the empty list. |
...
| `claims` | string[]                                          | N        | an array of claims that fulfill the requirements declared in an [Offering](#offering). If the creator of the RFQ wants to include no claims, then `claims` SHOULD be omitted rather than being an empty list. If `claims` is an empty list, the salted hash of the empty list must match `claimsHash`. |
